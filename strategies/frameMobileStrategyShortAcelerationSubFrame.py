...
class FrameMobileStrategyShortAcelerationSubFrame(StrategyBase):

    # Moving average parameters
    # params = (('pfast',20),('pslow',50),)

    def __init__(self, index, phemex_automation = None, parent = None):
        StrategyBase.__init__(self, index=index)

        self.parent = parent
        #self.log("Using FrameMobileStrategyShortAcelerationSubFrame" + str(index))
        self.lendata1 = 0
        ...
        self.name = "FrameMobileStrategyShortAcelerationSubFrame" + str(index)
        self.index = index
        self.buy_price = 0
    ...
    def next(self):
        ...
        #if MULTIPLE_INSTANCE == False:
        #    self.log(str(close) +  " en index " + str(self.index))
                    ...
                    if self.touch_media_low_iterada_3 == False:
                        ...
                        if self.delta_aceleration == False:
                            ...
                            if self.pre_buy == False and self.trade_made == False and self.delta_high_mean_3_succes(actual_price) == True:
                                ...
                                message = "pre- long buy: " + str(actual_price) +  " en index " + str(self.index)
                                ...
                                message = "pre - long limit = " + str(pre_long_limit) + " pre long limit objective = " + str(pre_long_limit_to ) +  " en index " + str(self.index)
                                #self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                                if (self.pre_long == False):
                                    ...
                                    self.pre_long_stop_loss = actual_price * (1 - 0.01)
        ...
        ############################################
                    ...
                    if self.touch_media_high_iterada_3_short == False:
                        ...
                        if self.delta_aceleration_short == False:
                            ...
                            if self.pre_buy == False and self.trade_made == False and self.delta_low_mean_3_succes_short(actual_price) == True:
                                ...
                                #self.buy_tick = self.total_ticks
                                message = "pre - short sell: " + str(actual_price) +  " en index " + str(self.index)
                                #self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                                pre_short_limit = actual_price * (1.0045)
                                pre_short_limit_to = pre_short_limit * (1 - 0.02) 
                                message = "pre - short limit = " + str(pre_short_limit) + " pre short limit objective = " + str(pre_short_limit_to) +  " en index " + str(self.index)
                                #self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                                if (self.pre_short == False):
                                    ...
                                    self.pre_short_stop_loss = actual_price * (1.01)
                                self.jsonParser.set_subida_estrepitosa(1, self.total_ticks)
                                if ENV == PRODUCTION and TESTING_PRODUCTION == False and LIVE == True:
                                    pass
            ...
            if self.pre_long == True:
                if actual_price <= self.pre_long_limit and self.pre_long_touch_limite == False and self.parent.long_active == False:
                    self.pre_long_touch_limite = True
                    self.parent.long_active = True
                    self.pre_long_limit_to = actual_price * 1.02
                    self.pre_long_stop_loss = actual_price * (1 - 0.0055)
                    message = "LONG buy: " + str(actual_price) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                    message = "Profit 1.02 = " + str(self.pre_long_limit_to) + " StopLoss  = " + str(self.pre_long_stop_loss) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                
                if actual_price >= self.pre_long_limit_to and self.pre_long_touch_limite == False:
                    self.pre_long = False
                    message = "Cancel pre long, touch limit to " + str(actual_price) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                
                # si ya hay uno activo cancela cuando se cumple condicion. 
                if actual_price <= self.pre_long_limit and self.pre_long_touch_limite == False and self.parent.long_active == True:
                    self.pre_long = False
                    message = "Cancel pre long, touch limit to " + str(actual_price) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                ...
                if actual_price >= self.pre_long_limit_to and self.pre_long_touch_limite == True:
                    self.pre_long = False
                    self.pre_long_touch_limite = False
                    self.parent.long_active = False
                    self.parent.total_long_orders += "T" 
                    message = "SUCCES LONG! price= " + str(actual_price) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                    message = self.parent.total_short_orders + "-" + self.parent.total_long_orders 
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                ...
                if actual_price <= self.pre_long_stop_loss and self.pre_long_touch_limite == True:
                    ...
                    self.pre_long_touch_limite = False
                    self.parent.long_active = False
                    self.parent.total_long_orders += "F" 
                    message = "LOST LONG. price =" + str(actual_price) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                    message = self.parent.total_short_orders + "-" + self.parent.total_long_orders
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
            ...
            if self.pre_short == True:
                if actual_price >= self.pre_short_limit and self.pre_short_touch_limite == False and self.parent.long_active == False:
                    self.pre_short_touch_limite = True
                    self.parent.long_active = True
                    self.pre_short_limit_to = actual_price * (1 - 0.02)
                    self.pre_short_stop_loss = actual_price * 1.0055
                    message = "SHORT sell: " + str(actual_price) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                    message = "Profit 1.02 = " + str(self.pre_short_limit_to) + " StopLoss  = " + str(self.pre_short_stop_loss) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                
                if actual_price <= self.pre_short_limit_to and self.pre_short_touch_limite == False:
                    self.pre_short = False
                    message = "Cancel pre short, touch limit to " + str(actual_price) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                ...
                if actual_price <= self.pre_short_limit_to and self.pre_short_touch_limite == True:
                    ...
                    self.pre_short_touch_limite = False
                    self.parent.long_active = False
                    self.parent.total_short_orders += "T"
                    message = "SUCCES SHORT! price= " + str(actual_price) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                    message = self.parent.total_short_orders + "-" + self.parent.total_long_orders
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                ...
                if actual_price >= self.pre_short_stop_loss and self.pre_short_touch_limite == True:
                    ...
                    self.pre_short_touch_limite = False
                    self.parent.long_active = False
                    self.parent.total_short_orders += "F"
                    message = "LOST SHORT. price =" + str(actual_price) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
                    message = self.parent.total_short_orders + "-" + self.parent.total_long_orders 
                    self.log(message,  to_ui = True, date = self.datetime[0], send_telegram=True)
            ...
            if self.is_order == True and self.trade_made == False and self.pre_sell == False and finish_trade == True:
                ...
                if do_sell == True:
                    ...
                    message = "Binance sell: " + str(actual_price) +  " en index " + str(self.index)
                    self.log(message,  to_ui = True, date = self.datetime[0])
                    do_sell = False
                    self.is_order = False
                    if ENV == PRODUCTION and TESTING_PRODUCTION == False and LIVE == True:
        ...
        if len(self.data1.low) > self.lendata1:
            ...
            if (self.indicators_ready):
                if self.index == 0:
                    ...
                    self.parent.ensambleIndicatorsFirstNode = self.ensambleIndicators
                else:
                    self.ensambleIndicators = self.parent.ensambleIndicatorsFirstNode
                    #print("Actualiza con Indicadores de first node " + str(self.index))
                #self.indicators_ready = True
                #if MULTIPLE_INSTANCE == False:
                #    print("New Indicators Ready!" +  " en index " + str(self.index))
                self.refresh()
