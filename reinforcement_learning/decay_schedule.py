class LinearDecaySchedule(object):
    def __init__(self, initial_value, final_value, max_steps):
        assert initial_value > final_value, "El valor inicial tiene que ser estrictamente mayor que el final."
...
if __name__ == "__main__":
    ...
    linear_schedule =LinearDecaySchedule(initial_value = epsilon_initial, 
                                        ...
                                        max_steps = 0.5 * MAX_NUM_EPISODES * STEPS_PER_EPISODES)
    epsilons = [linear_schedule(step) for step in range(MAX_NUM_EPISODES * STEPS_PER_EPISODES)]
    plt.plot(epsilons)
    plt.show()
