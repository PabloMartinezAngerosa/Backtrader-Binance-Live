...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df):
        ...
        self.observation_space = spaces.Dict({
            ...
            "Estimators": spaces.Box(low=0, high=1, shape=(4,), dtype=np.float16),
            "Actual": spaces.Box(low=self.delta_space * -1 , high=self.delta_space, shape=(1,), dtype=np.float16), # crop
            "Balance": spaces.Box(low=self.delta_space * -1, high=self.delta_space, shape=(1,), dtype=np.float16) # crop
        })
    ...
    def _next_observation(self):
        ...
        actual_price = self._get_current_price(False)
        # Actual price
        delta_price = actual_price - self.inicial_price
        # CROP DELTA PRICE MAS 300 MIN -300
        if delta_price > self.delta_space:
            delta_price = self.delta_space
        if delta_price < (self.delta_space * -1):
            delta_price = self.delta_space * -1

        actual_price = np.array([delta_price])

        # Balance
        balance = self.balance
        delta_balance = balance - self.inicial_balance
        # CROP DELTA PRICE MAS 300 MIN -300
        if delta_balance > self.delta_space:
            delta_balance = self.delta_space
        if delta_balance < (self.delta_space * -1):
            delta_balance = self.delta_space * -1
    ...
    def step(self, action):
        ...
        if self.current_candle == MAX_CANDLE_PER_EPISODES:
            done = True
            self.current_candle = 0
